...
module.exports = function (app) {
    ...
    plugin.start = function (options) {
        ...
        var hasDepth = db.prepare('select count(*) as count from pragma_table_info(?) where name=?').get('buffer', 'depth').count;
    ...
    plugin.schema = {
        ...
        required: ['trackInterval', 'minimumSpeed', 'minimumDistance', 'simplificationTolerance'],
        properties: {
            gpsSource: {
    ...
    function processDelta(delta) {
        ...
        var distance = getDistanceFromLatLonInMeters(lastRecordedPosition.latitude, lastRecordedPosition.longitude, position.latitude, position.longitude);
        var isNewDay = new Date(position.ts).getDate() != new Date(lastRecordedPosition.ts).getDate();
        var minutesSinceLastRecordedPosition = (Date.now() - lastRecordedPosition.ts) / 1000 / 60;
        ...
        app.debug(`bufferCount=${bufferCount} sog=${(sog || 0).toFixed(2)} distance=${(distance || 0).toFixed(2)} isNewDay=${isNewDay} minutesSinceLastRecordedPosition=${(minutesSinceLastRecordedPosition || 0).toFixed(2)} vesselStopped=${vesselStopped} `);
        ...
        if (!recordVoyage
            ...
            try {
                writeDailyGpxFile(getYyyymmdd(new Date(Date.now() - 24 * 60 * 60 * 1000))); // use yesterdays date
    ...
    function validPosition(position) {
        if (!position || !position.latitude || !position.longitude
            ...
            || (Math.abs(position.latitude) <= 0.01 && Math.abs(position.longitude) <= 0.01)
        ) {
            return false;
        }
        return true;
